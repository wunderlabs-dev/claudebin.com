  ...
  return (
    <Fragment>
      {block.questions.map((question) => {
        ...
        const isUserAnswer = answers.filter((answer) => not(options.includes(answer)));
              ...
              {question.options.map((option) =>
                <div
                  ...
                  className={cn(
                    ...
                    selected.includes(option.label) ? "bg-gray-100 text-white" : "text-gray-450 line-through",
                  )}
                >
                  {option.label}
                </div>
              )}
              {isUserAnswer.map((answer) => (
                <div
                  key={answer}
